...
# Correct mappings: English slug -> German name in database
CORRECT_MAPPINGS = {
    'toluene': 'Methylbenzol',
    'butanone-mek': 'Methylethylketon (MEK)',
    'heptane': 'Heptan, n-',
    'hexane': 'Hexan, n-',
    'perchloric-acid': 'Perchlorsäure',
    'turpentine-substitute': 'Terpentinersatz',
    'copper-vitriol-copper-sulfate': 'Kupfervitriol',
    'hydrofluoric-acid': 'Flusssäure',
    'motor-oil': 'Motorenöl',
    'hydraulic-oil': 'Hydrauliköle (Mineralölbasis)',
    'diethyl-ether': 'Diethylether',
    'glycolic-acid': 'Glykolsäure',
    'ammonia': 'Ammoniak, flüssig',
    'chlorine-bleach-solution': 'Chlorbleichlauge',
    'labarraque-solution-sodium-hypochlorite': 'Natriumhypochlorit',
...
for en_slug, de_name in CORRECT_MAPPINGS.items():
    # Find chemical in database
    chem_data = None
    for chem in data:
        if chem['name'] == de_name or de_name in chem['name']:
            chem_data = chem
            break
    
    if not chem_data:
        print(f"⚠️  {en_slug}: '{de_name}' not found in database")
        continue
    
    ratings_data = chem_data.get('ratings', {})
    if not ratings_data:
        print(f"⚠️  {en_slug}: No ratings data")
        continue
    ...
    if not os.path.exists(chem_path):
        print(f"⚠️  {en_slug}: Directory doesn't exist")
        continue
    
    # Regenerate all material pages
...
<body class="text-gray-700 min-h-screen">
    ...
    fixed += 1
    print(f"✅ Fixed {en_slug} → {de_name}")

print(f"\n✅ Fixed {fixed} chemicals, {pages_updated} pages updated")
