// Chemical Resistance App v2 - Spanish Version
// 1,600+ chemicals × 24 materials
// Uses chemicalTranslations from chemical_translations_es.js (1,496 translations)

const materialInfo = {
    // Termoplásticos
    "HDPE": { name: "HDPE", full: "Polietileno de alta densidad", type: "Termoplástico" },
    "LDPE": { name: "LDPE", full: "Polietileno de baja densidad", type: "Termoplástico" },
    ...
    "PSU": { name: "PSU", full: "Polisulfona", type: "Termoplástico" },
    "PVC_HART": { name: "PVC rígido", full: "PVC rígido (no plastificado)", type: "Termoplástico" },
    "PVC_WEICH": { name: "PVC flexible", full: "PVC flexible (plastificado)", type: "Termoplástico" },
    "SAN": { name: "SAN", full: "Estireno-Acrilonitrilo", type: "Termoplástico" },
    // Fluoropolímeros
    ...
    "PTFE": { name: "PTFE", full: "Politetrafluoroetileno (Teflón®)", type: "Fluoropolímero" },
    "PVDF": { name: "PVDF", full: "Fluoruro de polivinilideno (Kynar)", type: "Fluoropolímero" },
    // Elastómeros
    "EPDM": { name: "EPDM", full: "Etileno-Propileno-Dieno Monómero", type: "Elastómero" },
    "FPM": { name: "FPM/FKM", full: "Fluoroelastómero (Viton®)", type: "Elastómero" },
    "NBR": { name: "NBR", full: "Caucho nitrilo (Buna-N)", type: "Elastómero" },
    "SI": { name: "Silicona", full: "Caucho de silicona (VMQ)", type: "Elastómero" },
    // Metales
    "AL": { name: "Aluminio", full: "Aluminio", type: "Metal" },
    "V2A": { name: "Inox 304", full: "Acero Inoxidable 1.4301 (304)", type: "Metal" },
    "V4A": { name: "Inox 316", full: "Acero Inoxidable 1.4401 (316)", type: "Metal" },
};
...
const ghsImages = {
    ...
    'GHS02': 'img/ghs/ghs02.svg', // Inflamable
    'GHS03': 'img/ghs/ghs03.svg', // Comburente
    'GHS04': 'img/ghs/ghs04.svg', // Gas a presión
    'GHS05': 'img/ghs/ghs05.svg', // Corrosivo
    'GHS06': 'img/ghs/ghs06.svg', // Tóxico
    'GHS07': 'img/ghs/ghs07.svg', // Nocivo
    'GHS08': 'img/ghs/ghs08.svg', // Peligro para la salud
    'GHS09': 'img/ghs/ghs09.svg', // Peligroso para el medio ambiente
};

// Map old EU hazard codes to GHS pictograms (Spanish labels)
const euToGhs = {
    'T+': { ghs: 'GHS06', label: 'Muy tóxico' },
    'T': { ghs: 'GHS06', label: 'Tóxico' },
    'C+': { ghs: 'GHS05', label: 'Muy corrosivo' },
    'C': { ghs: 'GHS05', label: 'Corrosivo' },
    'Xn': { ghs: 'GHS07', label: 'Nocivo' },
    'Xi': { ghs: 'GHS07', label: 'Irritante' },
    'F+': { ghs: 'GHS02', label: 'Extremadamente inflamable' },
    'F': { ghs: 'GHS02', label: 'Fácilmente inflamable' },
    'O': { ghs: 'GHS03', label: 'Comburente' },
    'N': { ghs: 'GHS09', label: 'Peligroso para el medio ambiente' },
};
...
async function init() {
    try {
        ...
        document.getElementById('searchInput').placeholder = 'Error al cargar datos...';
    }
}
...
function translateName(germanName) {
    ...
    // Use comprehensive translations from chemical_translations_es.js
...
function getDisplayName(c) {
    // Spanish version - try to show Spanish name
    return translateName(c.name);
}

function getSecondaryName(c) {
    const es = translateName(c.name);
    // Show original German name as secondary if translated
    return es !== c.name ? c.name : '';
}

// Spanish concentration terms
function translateConc(conc) {
    if (!conc) return 'Puro / Sin diluir';
    // Translate common German concentration terms to Spanish
    return conc
        .replace(/rein/gi, 'puro')
        .replace(/unverdünnt/gi, 'sin diluir')
        .replace(/konzentriert/gi, 'concentrado')
        .replace(/verdünnt/gi, 'diluido')
        .replace(/gesättigt/gi, 'saturado')
        .replace(/wässrig/gi, 'acuoso')
...
function showSearchResults(query) {
    ...
    if (sortedMatches.length > 0) {
        ...
        searchResults.innerHTML = '<div class="px-4 py-3 text-gray-500">No se encontraron productos químicos</div>';
        searchResults.classList.remove('hidden');
    }
}
...
function showResultsForGroup(indices) {
    ...
    if (indices.length > 1) {
        ...
        concContainer.innerHTML = `
            <span class="font-semibold text-amber-800">Concentración:</span>
            ...
            <span class="text-xs text-amber-600 ml-2">← Seleccionar para la tabla</span>
        `;
        ...
        if (conc && conc !== 'Puro / Sin diluir') {
            concContainer.innerHTML = `<span class="font-semibold text-amber-800">Concentración:</span> <span class="font-bold text-amber-900">${conc}</span>`;
            concWrapper.style.display = 'inline-flex';
        } else {
...
function updateRatingsTable() {
    ...
    if (materialsToShow.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" class="py-6 text-center text-gray-500">No hay materiales con calificación ${minRating === '1' ? 'A' : 'A/B'} para este químico. Pruebe a cambiar el filtro.</td></tr>`;
    } else {
    ...
    if (recMaterials) {
        recMaterials.textContent = recommended.length > 0 
            ? recommended.join(', ') 
            : 'Ver calificaciones abajo - compatibilidad limitada';
    }
