...
<head>
    ...
    <meta name="description" content="Is {mat_name} resistant to {chem_name}? Rating: {r20[1]} at 20°C, {r50[1]} at 50°C. Complete compatibility guide.">
    ...
    <header class="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
            <a href="/" class="flex items-center gap-2">
                ...
                <span class="font-bold text-gray-900">ChemicalResistance.org</span>
            </a>
    ...
    <section class="bg-gradient-to-b from-emerald-50 to-white px-4 py-8">
        <div class="max-w-4xl mx-auto">
            <div class="flex items-center gap-2 text-sm text-emerald-600 mb-3">
                ...
                <a href="/chemicals/{en_slug}">{chem_name}</a><span>›</span>
        ...
        <div class="bg-white rounded-2xl shadow-sm border p-6 mb-6">
            ...
            <div class="space-y-4">
                <div class="border-b pb-4">
                    ...
                    <p class="text-gray-600">{mat_name} has <strong>{r20[1]} ({r20[0]})</strong> resistance to {chem_name} at 20°C.</p>
                </div>
                <div>
                    <h3 class="font-semibold mb-2">Can I store {chem_name} in {mat_name}?</h3>
                    <p class="text-gray-600">{"Yes, " if r20[0] in ['A','B'] else "Caution - "}{mat_name} is rated {r20[0]} for {chem_name}.</p>
                </div>
            </div>
        </div>
        <div class="grid md:grid-cols-2 gap-4">
...
print(f"✅ Updated {updated}/{total} pages with actual ratings")
print(f"📊 Coverage: {len(EN_TO_DE)} chemicals mapped")
