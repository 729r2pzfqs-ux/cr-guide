...
for chem in data:
    if chem['name'] not in FINAL_5:
        continue
    
    en_slug = FINAL_5[chem['name']]
    
    if os.path.exists(f'chemicals/{en_slug}'):
        print(f"Skipping {en_slug} - already exists")
        continue
...
<body class="text-gray-700 min-h-screen">
    ...
    added += 1
    print(f"✅ Added {en_slug}")

print(f"\n✅ Total: {added} chemicals, {pages} pages")
